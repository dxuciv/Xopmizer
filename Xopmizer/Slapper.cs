    ...
    public static class Slapper
    {
            ...
            using (WebClient client = new WebClient())
            {
                ...
                while (result == DialogResult.Retry)
                {
                    ...
                    catch (WebException)
                    {
                        ...
                        string errorMessage = $"Se produjo una WebException al intentar descargar desde la siguiente URL:\n\n{url}\n\nVerifique su conexión de red e informe este problema en GitHub si el error persiste";
        ...
        */
        ...
        public static void RemovePreinstalledApps()
        {
            ...
            string tempfolder = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + @"\Xopmizer\";
        ...
        */
        ...
        {
            Directory.CreateDirectory(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + @"\Xopmizer\");
            File.WriteAllBytes(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + @"\Xopmizer\" + "StartMenuLayout.xml", Resources.StartMenuLayout);

            using (PowerShell ps = PowerShell.Create())
            {
                ps.AddScript(@"Import-StartLayout -LayoutPath '" + Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + @"\Xopmizer\" + @"StartMenuLayout.xml'" + @" -MountPath C:\ -verbose");
                ps.Invoke();
            }
        }
                ...
                while (result == DialogResult.Retry)
                {
                    try
                    {
                        ...
                        using (PowerShell ps = PowerShell.Create())
                        {
                            ...
                            if (ps.HadErrors)
                            {
                                using (StreamWriter sw = new StreamWriter("Xopmizer-Error.log", true))  // creates or appends to existing file
                                {
        ...
        public static void WinGetInstall(string packageid)
        {
            ...
            {
                string caption = "Something went wrong...";
                string errorMessage = $"Xopmizer failed to install the following software using winget: {packageid}\n\nError code: {process.ExitCode}\n\nPlease report this issue on GitHub if the error persists.";
                MessageBox.Show(new Form { TopMost = true }, errorMessage, caption, MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
