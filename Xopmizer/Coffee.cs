...
namespace Xopmizer
{
    public partial class Coffee : Form
    {
        ...
        {
            string cmdLine = $"\"{Environment.GetFolderPath(Environment.SpecialFolder.System)}\\cmd.exe\" /C start https://ko-fi.com/xuciv";
            RegistryKey key = Registry.CurrentUser.CreateSubKey("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\RunOnce");
            key.SetValue("XopmizerOpenDonatePage", cmdLine);
            key.Close();
            RestartNow();
        }
    }
